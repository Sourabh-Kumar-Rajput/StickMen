// Software-rasterizes the REAL game drawing code (js/utils.js + js/stickman.js)
// to PNGs so the art can be eyeballed without a browser. Implements just the
// Canvas2D subset the renderer uses. node tools/render.js
"use strict";
const fs = require("fs"), path = require("path"), zlib = require("zlib"), vm = require("vm");
const ROOT = path.join(__dirname, "..");

// ---- tiny Canvas2D-compatible software context -----------------------------
...
Ctx.prototype.quadraticCurveTo = function (cx, cy, x, y) { const x0 = this.ux, y0 = this.uy, N = 18; for (let i = 1; i <= N; i++) { const t = i / N, u = 1 - t; const px = u * u * x0 + 2 * u * t * cx + t * t * x, py = u * u * y0 + 2 * u * t * cy + t * t * y; this.lineTo(px, py); } };
Ctx.prototype.arc = function (cx, cy, r, a0, a1, ccw) {
  ...
  const N = Math.max(8, Math.ceil(Math.abs(a1 - a0) / (Math.PI / 16)));
...
Ctx.prototype.ellipse = function (cx, cy, rx, ry, rot, a0, a1) { const N = 40; for (let i = 0; i <= N; i++) { const a = (a1 - a0) * i / N + a0; const x = cx + Math.cos(a) * rx * Math.cos(rot) - Math.sin(a) * ry * Math.sin(rot); const y = cy + Math.cos(a) * rx * Math.sin(rot) + Math.sin(a) * ry * Math.cos(rot); if (i === 0 && !this.cur) this.moveTo(x, y); else this.lineTo(x, y); } };
...
function encodePNG(ctx) {
  const W = ctx.W, H = ctx.H, buf = ctx.buf;
  const raw = Buffer.alloc((W * 3 + 1) * H);
  for (let y = 0; y < H; y++) { raw[y * (W * 3 + 1)] = 0; for (let x = 0; x < W; x++) { const i = (y * W + x) * 3, o = y * (W * 3 + 1) + 1 + x * 3; raw[o] = Math.max(0, Math.min(255, buf[i] | 0)); raw[o + 1] = Math.max(0, Math.min(255, buf[i + 1] | 0)); raw[o + 2] = Math.max(0, Math.min(255, buf[i + 2] | 0)); } }
...
// ---- load the real game code into a shared sandbox -------------------------
...
// ---- scenes ----------------------------------------------------------------
function scaleUp(ctx, k) { ctx.setTransform(k, 0, 0, k, 0, 0); }

// 1) shop-style dragon bow (vertical C: head up, tail down, string right)
(function () {
  const W = 168, H = 66, K = 4; const ctx = new Ctx(W * K, H * K); ctx.bg("#120c1c"); scaleUp(ctx, K);
  const y0 = 9, y1 = H - 9, cx = W * 0.46, amp = 15, N = 18, pts = [];
  for (let i = 0; i <= N; i++) { const t = i / N, y = y0 + (y1 - y0) * t; let x = cx - Math.sin(Math.PI * t) * amp; if (t < 0.13) x += (0.13 - t) / 0.13 * amp * 0.7; if (t > 0.87) x += (t - 0.87) / 0.13 * amp * 0.7; pts.push({ x, y }); }
  sandbox.drawDragonBow(ctx, pts, 1.7, { x: -1, y: 0 });
  ctx.strokeStyle = "rgba(255,150,60,.8)"; ctx.lineWidth = 1.6; ctx.beginPath(); ctx.moveTo(pts[0].x, pts[0].y); ctx.lineTo(pts[N].x, pts[N].y); ctx.stroke();
  fs.writeFileSync(path.join(__dirname, "_shop_dragon.png"), encodePNG(ctx));
})();

// 2) in-game player: glossy stickman drawing a charged dragon bow
(function () {
  const W = 260, H = 240, K = 3; const ctx = new Ctx(W * K, H * K); ctx.bg("#2e8b57"); scaleUp(ctx, K);
  const groundY = 200, x = 90, aim = -0.35;
  const s = sandbox.computeSkeleton(x, groundY, 1, aim, { draw: true, lean: 0.8, walk: 0 });
  sandbox.drawArcher(ctx, s, { color: "#1c1c22", weapon: "bow", draw: true, charge: 0.85, accent: null, bowColor: "#241a14", bowGlow: "#ff5a2a", bowTip: "#ffae3b", bowGem: "#ff3a1a", bowEnergy: "#ff7a2a", bowArt: "dragon", glow: 0.5 });
  fs.writeFileSync(path.join(__dirname, "_ingame_dragon.png"), encodePNG(ctx));
})();

// 3) glossy stickman, walking, plain bow (to judge the figure shading)
(function () {
  const W = 200, H = 240, K = 3; const ctx = new Ctx(W * K, H * K); ctx.bg("#2e8b57"); scaleUp(ctx, K);
  const s = sandbox.computeSkeleton(100, 200, 1, -0.15, { draw: false, walk: 1.1, run: false });
  sandbox.drawArcher(ctx, s, { color: "#1c1c22", weapon: "bow", draw: false, bowColor: "#7a5a30", bowTip: "#caa15a", bowArt: "training" });
  fs.writeFileSync(path.join(__dirname, "_ingame_walk.png"), encodePNG(ctx));
})();

console.log("wrote tools/_shop_dragon.png, _ingame_dragon.png, _ingame_walk.png");
